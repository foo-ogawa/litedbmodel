 ...
 *
 * Usage:
 *   php orm_bench/main.php <dialect> [reps] [warmup]   # print the CSV
 *   php orm_bench/main.php safety <dialect>            # assert + print the safety statement counts
 *
...
if (($argv[0] ?? null) === 'safety') {
    OrmBench::safety($argv[1] ?? 'sqlite');
    return;
}

$dialect = $argv[0] ?? 'sqlite';
$reps = isset($argv[1]) ? (int) $argv[1] : 300;
$warmup = isset($argv[2]) ? (int) $argv[2] : 30;
OrmBench::measure($dialect, $reps, $warmup);
