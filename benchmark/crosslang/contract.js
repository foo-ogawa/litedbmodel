...
export const ORM_DIALECTS = ['sqlite', 'mysql', 'postgres'];
export const ORM_OPS = [
...
export const ORM_OP_IDS = ORM_OPS.map((o) => o.id);
export const ORM_WRITE_OP_IDS = new Set(ORM_OPS.filter((o) => o.write).map((o) => o.id));
export const ORM_OP_LABEL = Object.fromEntries(ORM_OPS.map((o) => [o.id, o.label]));
// ── The op axis — the 19 ORM-comparison ops (no subset) ──────────────────────
export const CROSSLANG_CASE_IDS = ORM_OP_IDS;
export const CROSSLANG_CASE_LABELS = ORM_OP_LABEL;
// Ops whose logical op is a WRITE (the report tags these `W`, reads `R`).
export const CROSSLANG_WRITE_CASES = ORM_WRITE_OP_IDS;
