...
const LIVE_FILES = [
  ...
  'test/integration/NamedDbCodegen.test.ts',
...
const OFFLINE_TESTS = [
  ...
  "test/integration/ReadmeConformance.test.ts › README conformance — sqlite README §Transactions — the body READS ITS OWN WRITES, so a dependent write can use that id (the procedural shape: precondition, parent→child ref and short-circuit written BY HAND)",
