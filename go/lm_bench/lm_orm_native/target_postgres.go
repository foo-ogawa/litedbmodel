//go:build bench_postgres

// The postgres target's generated module, bound to the SAME `op()` switch (#156). The SQL is baked per
// dialect, so each target is its OWN package (bc names it after the declaring class) and the cell
// selects one AT BUILD TIME — a `-tags bench_postgres` binary can only ever run postgres. The aliases below are
// direct function values: the call in `op()` resolves statically, with no lookup at run time.
package main

import g "github.com/foo-ogawa/litedbmodel/go/v3/lm_bench/lm_orm_native/gen/benchpostgres"
