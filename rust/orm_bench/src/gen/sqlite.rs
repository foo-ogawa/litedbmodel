...
pub fn nestedRelations(
    ...
    *cell_n0.borrow_mut() = match wire_n0.as_list() {
    ...
    if produced_n3.get() {
        let payload_n4 = WireRow { entries: vec![("bigint".to_string(), WireValue::Bool(true)), ("params".to_string(), WireValue::List(WireList { items: { let __v: Vec<WireValue> = vec![WireValue::List(WireList { items: cell_n3.borrow().clone() })]; __v } })), ("returning".to_string(), WireValue::Bool(false)), ("sql".to_string(), WireValue::Str("SELECT id, body, post_id FROM benchmark_comments WHERE post_id IN (SELECT value FROM json_each(?)) ORDER BY id ASC".to_string())), ("write".to_string(), WireValue::Bool(false))] };
...
pub fn compositeRelations(
    ...
    *cell_n0.borrow_mut() = match wire_n0.as_list() {
    ...
    // ── op 'n4' (executeSQL, parent:n3) ──
    if produced_n3.get() {
        let payload_n4 = WireRow { entries: vec![("bigint".to_string(), WireValue::Bool(true)), ("params".to_string(), WireValue::List(WireList { items: { let __v: Vec<WireValue> = vec![WireValue::List(WireList { items: cell_n3.borrow().clone() })]; __v } })), ("returning".to_string(), WireValue::Bool(false)), ("sql".to_string(), WireValue::Str("SELECT tenant_id, comment_id, post_id, body FROM benchmark_tenant_comments WHERE EXISTS (SELECT 1 FROM json_each(?) je WHERE json_extract(je.value, '$[0]') = benchmark_tenant_comments.tenant_id AND json_extract(je.value, '$[1]') = benchmark_tenant_comments.post_id) ORDER BY comment_id ASC".to_string())), ("write".to_string(), WireValue::Bool(false))] };
        let wire_n4 = match execute_sql(payload_n4) {
            Ok(r) => r,
            Err(e) => return Err(op_failed("n4", "fail", e)),
        };
