...
pub fn nestedFindAll(
    ...
    *cell_n0.borrow_mut() = match probe_list_at(Some(wire_n0)) {
    ...
    if produced_n1.get() {
        let payload_n2 = WireRow { entries: vec![(Cow::Borrowed("bigint"), WireValue::Bool(false)), (Cow::Borrowed("params"), WireValue::List(WireList { items: { let __v: Vec<WireValue> = vec![WireValue::List(WireList { items: cell_n1.take() })]; __v } })), (Cow::Borrowed("returning"), WireValue::Bool(false)), (Cow::Borrowed("sql"), WireValue::Str(Cow::Borrowed("SELECT id, title, author_id FROM benchmark_posts WHERE benchmark_posts.author_id = ANY(?::@@PG_ARRAY_CAST@@) ORDER BY id ASC"))), (Cow::Borrowed("write"), WireValue::Bool(false))] };
...
pub fn nestedRelations(
    ...
    *cell_n0.borrow_mut() = match probe_list_at(Some(wire_n0)) {
    ...
    if produced_n3.get() {
        let payload_n4 = WireRow { entries: vec![(Cow::Borrowed("bigint"), WireValue::Bool(false)), (Cow::Borrowed("params"), WireValue::List(WireList { items: { let __v: Vec<WireValue> = vec![WireValue::List(WireList { items: cell_n3.take() })]; __v } })), (Cow::Borrowed("returning"), WireValue::Bool(false)), (Cow::Borrowed("sql"), WireValue::Str(Cow::Borrowed("SELECT id, body, post_id FROM benchmark_comments WHERE benchmark_comments.post_id = ANY(?::@@PG_ARRAY_CAST@@) ORDER BY id ASC"))), (Cow::Borrowed("write"), WireValue::Bool(false))] };
...
pub fn compositeRelations(
    ...
    *cell_n0.borrow_mut() = match probe_list_at(Some(wire_n0)) {
    ...
    // ── op 'n4' (executeSQL, parent:n3) ──
    if produced_n3.get() {
        let payload_n4 = WireRow { entries: vec![(Cow::Borrowed("bigint"), WireValue::Bool(false)), (Cow::Borrowed("params"), WireValue::List(WireList { items: { let __v: Vec<WireValue> = vec![WireValue::List(WireList { items: cell_n3.take() })]; __v } })), (Cow::Borrowed("returning"), WireValue::Bool(false)), (Cow::Borrowed("sql"), WireValue::Str(Cow::Borrowed("SELECT tenant_id, comment_id, post_id, body FROM benchmark_tenant_comments JOIN (SELECT (_t->>0)::int AS key0, (_t->>1)::int AS key1 FROM json_array_elements(?::json) AS _t) AS _keys ON benchmark_tenant_comments.tenant_id = _keys.key0 AND benchmark_tenant_comments.post_id = _keys.key1 ORDER BY comment_id ASC"))), (Cow::Borrowed("write"), WireValue::Bool(false))] };
        let wire_n4 = match execute_sql(payload_n4) {
            Ok(r) => r,
            Err(e) => return Err(op_failed("n4", "fail", e)),
        };
