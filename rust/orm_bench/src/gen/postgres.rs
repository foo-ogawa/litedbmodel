...
pub fn createMany(
    ...
    if produced_n0.get() {
        let payload_n2 = WireRow { entries: vec![(Cow::Borrowed("opts"), match Some(ExecOptions { guard: Option::<CapGuard>::None, returning: false, whereDynamic: Option::<DynamicWherePlan>::None, write: true }) { Some(ov0) => WireValue::Row(WireRow { entries: vec![(Cow::Borrowed("guard"), match ov0.guard { Some(ov1) => WireValue::Row(WireRow { entries: vec![(Cow::Borrowed("limit"), WireValue::Int(ov1.limit)), (Cow::Borrowed("model"), match ov1.model { Some(ov2) => WireValue::Str(ov2.into()), _ => WireValue::Null }), (Cow::Borrowed("relation"), WireValue::Str(ov1.relation.into()))] }), _ => WireValue::Null }), (Cow::Borrowed("returning"), WireValue::Bool(ov0.returning)), (Cow::Borrowed("whereDynamic"), match ov0.whereDynamic { Some(ov1) => WireValue::Row(WireRow { entries: vec![(Cow::Borrowed("frags"), WireValue::List(WireList { items: ov1.frags.into_iter().map(|e2| WireValue::Row(WireRow { entries: vec![(Cow::Borrowed("params"), WireValue::List(WireList { items: e2.params.into_iter().map(|e4| match e4 { Some(ov5) => ov5, _ => WireValue::Null }).collect() })), (Cow::Borrowed("skipped"), WireValue::Bool(e2.skipped)), (Cow::Borrowed("sql"), WireValue::Str(e2.sql.into()))] })).collect() }))] }), _ => WireValue::Null }), (Cow::Borrowed("write"), WireValue::Bool(ov0.write))] }), _ => WireValue::Null }), (Cow::Borrowed("params"), WireValue::List(WireList { items: { let __v: Vec<WireValue> = vec![WireValue::List(WireList { items: cell_n0.take().into_iter().map(|e0| WireValue::Str(e0.into())).collect() }), WireValue::List(WireList { items: cell_n1.take().into_iter().map(|e0| WireValue::Str(e0.into())).collect() })]; __v } })), (Cow::Borrowed("sql"), WireValue::Str(Cow::Borrowed("INSERT INTO benchmark_users (email, name) SELECT v.email, v.name FROM UNNEST(?::text[], ?::text[]) AS v(email, name)")))] };
        let wire_n2 = match execute_sql(payload_n2) {
            Ok(r) => r,
            Err(e) => return Err(op_failed("n2", "fail", e)),
        };
